...
from dataset.loadDataWeChat import Vocabulary
...
lrs, nums_epochs, device = [0.0005, 0.0001, 0.00001, 0.000001], [0, 0, 4000, 8000], 'cpu'
...
pretrain_vocab = Vocabulary('pretrain.tokenizer')
...
if __name__ == '__main__':
    ...
    # pretrain(DecoderOnlyNet, pretrain_data_iter, lrs, nums_epochs, device, edition, pre_train = f'parameters/MyTransformers-b4.params')

    # EncoderDecoderNet.load_state_dict(torch.load(f'MyTransformers-a.params'))
    # EncoderDecoderNet = EncoderDecoderNet.to(device)
    DecoderOnlyNet.load_state_dict(torch.load(f'parameters/MyTransformers-{edition}.params'))
    ...
    while True:
        ...
        reply, history, _ = predict(DecoderOnlyNet, query, '', pretrain_vocab, pretrain_num_steps, device)
        print(f'Nyte: {reply}\n')
